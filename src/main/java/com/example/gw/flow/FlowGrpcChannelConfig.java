...
public class FlowGrpcChannelConfig {
    ...
    public Map<String, GatewayCoreServiceGrpc.GatewayCoreServiceBlockingStub> flowStubs() {
        return loader.getConfig().getFlows().entrySet().stream()
                .collect(Collectors.toUnmodifiableMap(
                        Map.Entry::getKey,
                        e -> buildStub(e.getValue())
                ));
    }

    private GatewayCoreServiceGrpc.GatewayCoreServiceBlockingStub buildStub(FlowResource flow) {
        List<FlowResource.Target> targets = flow.getSpec().getLoadBalancing().getTargets();
        if (targets == null || targets.isEmpty()) {
            throw new IllegalStateException("Flow '" + flow.getMetadata().getName() + "' has no targets");
        }
        FlowResource.Target target = targets.get(0);
        if (targets.size() > 1) {
        ...
        ManagedChannel channel = ManagedChannelBuilder
                ...
                .usePlaintext()
                .build();
        log.debug("gRPC channel created for flow '{}' → {}:{}", flow.getMetadata().getName(), target.getHost(), target.getPort());
